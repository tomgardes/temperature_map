...
class TempMap:
    ...
    def run(self,index):
		...
		if result:
			...
			mntClip = processing.runalg("saga:resampling", mntExtent['OUTPUT'], True, 0,0, extent,100,outputPath1) #A CORRIGER AVEC UN IF (pb d extent)!
			iface.addRasterLayer(outputPath1, "MNT_CLIP")
			
			#Si utilisation des donnees MAPUCE 
			dataMapuce = []
			...
			resamplFracVille = processing.runalg("saga:resampling", outputCalc, True, 0,0, extent,100,None) #dimensionnement de la fraction de ville mapuce
			...
			lmr = processing.runalg("saga:multipleregressionanalysispointsgrids", inputs, stationsPathFinal,fieldTemp,0,True,True,0,5,5,None,None,None,outputPath3,outputPath2) #regression lineaire multiple
